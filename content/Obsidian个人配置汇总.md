...
最后是理工科朋友可能用的比较多的公式输入功能。主要参考[用 LaTeX 实时做课堂笔记](https://castel.dev/post/lecture-notes-1/)，在 ob 中引入 **LaTeX suite 插件**，可以做到像打字一样快的输入各种 LaTeX 符号。我个人使用的LaTeX 快捷输入模板供各位取用： [[latex.js]]。
...
- 并不是每一条划线都值得单独批注，有可能*一段中的多个划线共同阐述了一个观点*，如果只对这一族划线中的一个进行批注，则失去了一致性；
